...
class PostTagController extends Controller
{
    ...
    {
        $postTag = PostTag::orderBy('id', 'DESC')->paginate(100);
    ...
    public function store(Request $request)
    {
        ...
        if ($status) {
            request()->session()->flash('success', 'Đã thêm thẻ bài viết thành công');
        } else {
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($status) {
            request()->session()->flash('success', 'Đã cập nhật thẻ bài viết thành công');
        } else {
            request()->session()->flash('error', 'Please try again!!');
        }
        return redirect()->route('post-tag.index');
    ...
    public function destroy($id)
    {
        ...
        if ($status) {
            request()->session()->flash('success', 'Đã xóa thẻ bài viết thành công');
        } else {
